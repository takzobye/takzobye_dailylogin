...
function loadPlayerDailyLogin(xPlayer)
    ...
    local row = MySQL.single.await('SELECT daily_login FROM users WHERE identifier = ?', { xPlayer.getIdentifier() })
    ...
    if Config.dailyLogin.key == data.key then 
        ...
        MySQL.update.await('UPDATE users SET daily_login = NULL WHERE identifier = ?', { xPlayer.getIdentifier() })
    end
end
...
RegisterNetEvent('zobyeteam_dailylogin:receive', function(targetIndex)
    ...
    local affectedRow = MySQL.update.await('UPDATE users SET daily_login = ? WHERE identifier = ?', { json.encode(data), xPlayer.getIdentifier() })

    if not affectedRow then return end

    xPlayer.addInventoryItem(item.name, item.amount)

    TriggerClientEvent('zobyeteam_dailylogin:received', playerId, targetIndex)
end)